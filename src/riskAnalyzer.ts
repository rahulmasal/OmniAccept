...
export class RiskAnalyzer {
    ...
    private refreshWorkspaceFolders(): void {
        this.workspaceFolders = vscode.workspace.workspaceFolders ? [...vscode.workspace.workspaceFolders] : [];
    ...
    public analyze(action: ActionContext): RiskAnalysisResult {
        ...
        if (settings.trustedWorkspaceOnly && !settings.isCurrentWorkspaceTrusted()) {
        ...
        if (this.isSensitivePath(action)) {
            ...
            return { level: RiskLevel.High, reasons, overrideRule: 'sensitiveFileAccess' };
        }
        ...
        if (this.isExternalDirectoryAccess(action)) {
            ...
            return { level: RiskLevel.High, reasons, overrideRule: 'externalDirectoryAccess' };
        }
    ...
    private analyzeByActionType(action: ActionContext): RiskAnalysisResult {
        ...
        switch (action.type) {
            ...
            case ActionType.TerminalCommand:
                ...
                return { level: RiskLevel.High, reasons, overrideRule: 'terminalCommand' };
            ...
            case ActionType.McpToolAccess:
                ...
                return { level: RiskLevel.High, reasons, overrideRule: 'mcpToolAccess' };
    ...
    private matchSimpleGlobPart(pathPart: string, patternPart: string): boolean {
        ...
        if (patternPart === '*.env*') {
            return pathPart.startsWith('.env') || pathPart.includes('.env.');
        }
    ...
    private isExternalDirectoryAccess(action: ActionContext): boolean {
        if (action.type !== ActionType.ExternalDirectoryAccess && action.type !== ActionType.ReadFiles) {
            return false;
        }
     ...
     */
    private determineRiskLevel(reasons: string[], typeResult: RiskAnalysisResult): RiskAnalysisResult {
        // High risk reasons
    ...
    public dispose(): void {
        ...
        (RiskAnalyzer as unknown as { instance?: RiskAnalyzer }).instance = undefined;
    }
}
